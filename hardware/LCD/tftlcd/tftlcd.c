...
****************************************************************************/
#if 0
void LCD_ShowFontHZ(u16 x, u16 y, u8 *cn, u16 fontColor)	 
{  
	u8 i, j, wordNum;
	u16 color;
	while (*cn != '\0')
	{
		LCD_Set_Window(x, y, x+31, y+28);  //一个汉字占32(列)*29(行)个像素
		for (wordNum=0; wordNum<120; wordNum++)
		{	//wordNum表示第几个结构体，每个结构体包含了一个汉字的信息
			if ((CnChar32x29[wordNum].Index[0]==*cn)
			     &&(CnChar32x29[wordNum].Index[1]==*(cn+1))) //一个汉字占用2Bytes
			{
				for(i=0; i<116; i++) 
				{	//MSK的位数
			     ...
			     &&(CnChar32x29[wordNum].Index[1]==*(cn+1)))
			{
				...
				{	//MSK的位数
					...
					for(j=0;j<8;j++) 
					{
						if((color&0x80)==0x80)
						{
							LCD_DrawFRONT_COLOR(x,y,fontColor);
						} 						
						else
						{
...
//void LCD_ShowFontHZ24_19(u16 x, u16 y, u8 *cn){
//	u8 i, j, wordNum;
//	u16 color;
//	u16 x0=x; 
//	u16 y0=y; 
...
//		{	//wordNum扫描字库的字数
//			if ((CnChar24x19[wordNum].Index[0]==*cn)
//			     &&(CnChar24x19[wordNum].Index[1]==*(cn+1)))
//			{
//				for(i=0; i<57; i++) 
//				{	//MSK的位数
//					color=CnChar24x19[wordNum].Msk[i];
//					for(j=0;j<8;j++) 
//					{
//						if((color&0x80)==0x80)
//						{
